...
const Airdrop = () => {
  ...
  return (
    <div
      id="noscroller"
      className="w-full h-[90%] flex pt-4 items-center text-center font-[ageo] text-white"
    >
      {openInfo ? (
        ...
        <div className="w-full flex items-center flex-col gap-1 rounded-t-2xl bg-slate-900 border-t-4 border-yellow-500 mt-16 h-full p-4">
          ...
          <h3 className="text-xl font-[ageobold]">
            Most Awaited House Drop Launch <br />
          ...
          <button
            className=" mt-2 px-12 py-2 bg-yellow-400 text-slate-800 rounded-md"
  ...
  return (
    <div className="w-full flex justify-center items-center h-48 rounded-xl overflow-hidden mt-4">
      <img
        src={imageSrc}
        alt="Slideshow image"
        className="h-full border-2 rounded-lg border-yellow-500"
      />
    </div>
  );
};
...
const InformationPage: React.FC<InfoProps> = ({ setOpen, propertyData }) => {
  ...
  const dataHIW = [
    {
      title: "Hold GTPR",
      Info: "Ensure you have at least $9 worth of GTPR tokens to qualify",
    },
    {
      title: "Earn Points",
      Info: "Play to accumulate, which act as your bidding power",
    },
    {
      title: "Place your bid",
      Info: "Use points to bid for tokenized shares in the property. you can update your bid as many as you can to improve your winning position",
    },
    {
      title: "Claim Ownership",
      Info: "Winning bidders pay a processing fee in GTPR to recieve thier NFTs",
    },
    {
      title: "Own or trade",
      Info: "Use sell or leverage your NFT as valuable asset.",
    },
  ];
  const images = [
    {
  ...
  return (
    <>
      {openHowItWorks ? (
        <div
          id="noscroller"
          className="w-full flex items-center flex-col gap-2 rounded-t-2xl bg-slate-900 border-t-4 border-yellow-500 mt-16 h-full p-4"
        >
          <span className="w-full">
            <FaArrowLeft size={24} onClick={() => setOpen(false)} />
          </span>
          <h1 className="text-3xl font-[ageobold]">How it Works?</h1>
          <div
            id="noscroller"
            className="w-full h-[72vh] pb-12 overflow-y-scroll"
          >
            {dataHIW.map((item, idx) => {
              return (
                <div
                  key={idx}
                  className="mb-1 w-full min-h-24 bg-slate-800 rounded-lg p-2 flex gap-2"
                >
                  <div>
                    <p className="text-xl text-left px-2 font-[ageobold]">
                      {item.title}
                    </p>
            ...
            <button
              ...
              className="mt-2 px-6 py-2 bg-yellow-400 text-slate-800 rounded-md font-semibold"
            >
              Back to Game
            </button>
          </div>
